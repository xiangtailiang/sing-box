...
check_install() {
  ...
  # 并发下载订阅模板 (clash2 / sing-box-template),clash 改用内置自定义模板,不再外拉
  {
...
# Mihomo/Clash 配置
# Generated by sing-box.sh — 仅包含本机节点 + 自动更新的 rule-providers
...
rule-providers:
  ...
  la-extra:
    type: http
    behavior: classical
    url: "https://raw.githubusercontent.com/cuber/rule-set/main/Clash/LA-Extra.yaml"
    path: ./ruleset/la-extra.yaml
    interval: 86400
  cncidr:
    type: http
    behavior: ipcidr
...
rules:
  # 屏蔽 QUIC (UDP:443),强制回退 TCP,解决 Google Play 下载卡住
  - AND,((NETWORK,UDP),(DST-PORT,443)),REJECT
  # 让 rule-provider 自身的下载走代理,避免冷启动拉不动
